...
const browserTestFiles = [
  path.resolve(rootPath, 'test', 'browser-env.ts'),
  ...glob.sync(path.resolve(rootPath, 'test', '*.spec.ts'))
]
...
// code for client
app.get('/client.live.js', browserify(
  path.resolve(rootPath, 'src', 'client.ts')
))
